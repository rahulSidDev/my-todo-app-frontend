  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center bg-slate-100">
      ...
      <p className="text-lg text-gray-600 mb-8 text-center">
        Organize your tasks, boost productivity, and keep track of everything in one place.
      </p>

      <div className="flex gap-4">
        {isLoggedIn ? (
          <>
          <p className='font-bold text-4xl px-6 text-blue-950'>Welcome <i>{user.user[0].name}</i>,</p>
          <Link
            to="/dashboard"
          ...
          <>
            <Link to="/signup" className="px-6 py-3 bg-blue-950 text-white rounded-md">Get Started</Link>
            <Link to="/login" className="px-6 py-3 bg-white border border-blue-950 text-blue-950 rounded-md">Login</Link>
          </>
        )}
      </div>
